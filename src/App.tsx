...
function PrivateApp() {
  ...
  const EVENT_INSTRUCTION = 'Extract only actual ordinance appointment blocks with exactly these columns: "Date of event", "Time of event", "Room of event", "Type of event", "Names of the guests". Each appointment block is defined by one visible start time plus one room. Return exactly one row per unique Date + Time + Room appointment. Never split one appointment into multiple rows because it contains child-to-parent participants, children, parents, aliases, deceased markers, or multiple highlighted name lines. For "Date of event", find the large date header above the events, such as "Saturday, August 1, 2026", and repeat it for every event under that date until the next date header. For "Time of event", extract only the event start time at the left, such as "9:00 AM"; do NOT use the italic arrival time in parentheses. For "Room of event", extract the room label shown on the right, such as "Room 1", "Room 2", or "Room 3"; do NOT leave this blank when a room is visible. For "Type of event", extract the full ordinance type text shown in the appointment block, such as "LICENSED MARRIAGE", "SEALING AFTER CIVIL MARRIAGE", "CHILD-TO-PARENT SEALING", or a combined type when both appear in the same block. For "Names of the guests", include every person name that belongs to the same appointment block, including couple names, child-to-parent children, parent names, parenthetical preferred names, deceased markers, and "To Be Sealed" notes when shown. Join all names within that one appointment with "; ". Example: "Rivera Trevino, Jose Jaden (Rivera, Jaden); Jones, Aycie Ila (Jones, Aycie)". Ignore arrival times like "(7:45 AM Arrival)", guest counts like "40 Guests", notes, sealer lines, room totals, session total patrons lines, page headers, and page footers. The output must not contain two rows with the same Date of event + Time of event + Room of event.';
