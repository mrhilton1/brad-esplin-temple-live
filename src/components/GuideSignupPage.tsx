...
export default function GuideSignupPage() {
  ...
  const visibleSelectedSlots = selectedSlot ? [selectedSlot] : selectedSlots;
            ...
            <section className="space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <h2 className="text-lg font-black">{selectedDate ? formatDateHeading(selectedDate) : "Choose a date"}</h2>
                  {selectedSlot && (
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-black text-slate-600 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700"
                    >
                      Change time
                    </button>
                  )}
